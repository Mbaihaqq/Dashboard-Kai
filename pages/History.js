      ...
      <div className="w-full px-4 py-6 bg-[#F8F9FA] min-h-screen">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
            <div className="flex items-center gap-4 w-full md:w-auto">
                <button 
                    onClick={() => router.push('/')} 
                    className="p-2 bg-white border border-gray-200 rounded-lg text-gray-600 hover:text-[#005DAA] hover:border-[#005DAA] transition-all shadow-sm"
                >
                    <ArrowLeft size={20} />
                </button>
                <div>
                                    ...
                                    <tr key={idx} className="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <div className="flex items-center">
                                                <div className="flex-shrink-0 h-8 w-8 rounded-full bg-blue-100 flex items-center justify-center text-[#005DAA] border border-blue-200 text-xs font-bold">
                                                    {getInitials(row.admin_name)}
                                                </div>
                                                <div className="ml-3">
                                                    <div className="text-sm font-medium text-gray-900 truncate max-w-[200px]" title={row.admin_name}>
                                                        {row.admin_name || 'Admin'}
                                                    </div>
                                                    <div className="text-xs text-gray-500">Authorized Admin</div>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-center">
                                            <button 
                                                onClick={() => handleDelete(row.id, row.file_name)}
                                                disabled={isDeleting}
                                                className="p-2 bg-white border border-gray-200 rounded-lg text-gray-400 hover:text-red-600 hover:border-red-200 hover:bg-red-50 transition-all disabled:opacity-50"
                                                title="Hapus Log Ini"
                                            >
                                                <Trash2 size={16} />
                                            </button>
