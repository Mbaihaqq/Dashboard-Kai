  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      if (userId) {
        ...
        // Memberikan waktu agar sistem Auth dan Database sinkron
        await new Promise((resolve) => setTimeout(resolve, 2000));

        // 3. Simpan ke tabel profiles
        // Pastikan nama property (nip) sama dengan nama kolom di dashboard Anda
        const { error: profileError } = await supabase
          ...
          .insert([
            { 
              id: userId, 
              username: username, 
              nip: nip.trim(), // Memastikan NIP masuk tanpa spasi tambahan
              position: position, 
              role: 'user'
            }
          ]);
        ...
        if (profileError) {
          ...
          throw new Error(`Auth Berhasil, tapi profil gagal: ${profileError.message}`);
        }
      }

      alert("Registrasi Berhasil! Username, NIP, dan Jabatan telah tersimpan.");
            ...
            <form className="space-y-4 max-w-md" onSubmit={handleRegister}>
                <div>
                    ...
                    <input 
                        ...
                        className="w-full border border-gray-400 rounded-lg px-4 py-2.5 focus:outline-none focus:border-[#005DAA] focus:ring-1 focus:ring-[#005DAA]"
                    />
                </div>
                ...
                <div>
                    ...
                    <div className="relative">
                        <input 
                            ...
                            className="w-full border border-gray-400 rounded-lg px-4 py-2.5 pr-10 focus:outline-none focus:border-[#005DAA] focus:ring-1 focus:ring-[#005DAA]"
                        />
                        <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-3 text-gray-500 hover:text-black transition">
                ...
                <div className="pt-4 pb-12">
                    <button type="submit" disabled={loading} className="w-full bg-[#005DAA] text-white font-bold py-3 rounded-lg hover:bg-blue-800 transition shadow-md disabled:bg-gray-400">
